...
const _absUrl = (url, base) => new URL(url, base).toString();

function _build_json_feed(options) {
  return async (allPosts, meta, n = 10) => {
    ...
    if (!meta.home_page_url) {
      throw new Error("home_page_url is required by eleventy-plugin-json-feed");
    }
    ...
    if (meta.user_comment) {
      feed.user_comment = _require_string(meta.user_comment, "user_comment");
    }
    ...
    if (meta.icon) {
      feed.icon = _require_url(meta.icon, "icon");
    }
...
function configFunction(eleventyConfig, userOptions) {
  ...
  // jump through a hoop to make it possible to test
  const jsonFeed = _build_json_feed(options);
  eleventyConfig.addNunjucksAsyncShortcode("jsonFeed", jsonFeed);
}

module.exports = {
  buildJsonFeed: _build_json_feed,
  configFunction,
};
